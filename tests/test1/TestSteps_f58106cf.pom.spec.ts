...
test("Contacts - List page is displayed, search for contact and open profile", async ({ page, context }) => {
  ...
  await page.locator("//SPAN[normalize-space() = \"Approval Status (Contact)\"]").hover();
  await page.getByRole('link', { name: "Add Contact" }).hover();
  await page.getByPlaceholder("Contact Name or Email Address").fill("Fiona Alexander");
  await page.locator("BUTTON[type='button']").nth(2).click();
  await page.locator("//a[normalize-space()=\"Fiona Alexander\"]").click();
  await page.waitForLoadState('load');
