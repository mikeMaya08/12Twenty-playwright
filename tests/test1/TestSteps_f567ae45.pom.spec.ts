...
test("Employers - UI elements are visible - Admin", async ({ page, context }) => {
  ...
  await page.getByPlaceholder("Company Name").hover();
  await page.locator("//SPAN[normalize-space() = \"Approval Status\"]").hover();
  await page.getByRole('link', { name: "Add Employer" }).hover();
});
